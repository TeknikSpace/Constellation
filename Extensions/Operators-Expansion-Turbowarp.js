((Scratch) => {
    ...
    class pmOperatorsExpansion {
      getInfo() {
        return {
          ...
          docsURI: "https://extensions.penguinmod.com/",
          ...
          blocks: [
            ...
            {
                opcode: 'getContentOnLine',
                text: 'get text on [lineNumber] in [text]',
                blockType: Scratch.BlockType.REPORTER,
                disableMonitor: true,
                arguments: {
                    text: {
            ...
            {
                opcode: 'getNumberOfLines',
                text: 'get amount of lines in [text]',
                blockType: Scratch.BlockType.REPORTER,
                disableMonitor: true,
                arguments: {
                    text: {
    ...
    getContentOnLine(args) {
        const lines = args.text.split('\n');
        if (args.lineNumber < 1 || args.lineNumber > lines.length) {
            return "Invalid line number";
        }
        return lines[args.lineNumber - 1];
    
    }

    getNumberOfLines(args) {
        const lines = (args.text).split('\n');
        return lines.length; 
    }
    textToLines(args) {
